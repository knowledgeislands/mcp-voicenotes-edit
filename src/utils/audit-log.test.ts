...
describe('appendAuditEvent / withAuditLog (mcp-voicenotes-edit)', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_PATH = logPath
    delete process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG
    delete process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_MAX_BYTES
    delete process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_KEEP
  })

  afterEach(async () => {
    await fs.rm(tmpDir, { recursive: true, force: true })
    delete process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_PATH
    delete process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG
    delete process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_MAX_BYTES
    delete process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_KEEP
  })
  ...
  it('returns the handler verbatim for read-level tools in default writes mode', async () => {
    ...
    expect(withAuditLog('voicenotes_note_get', 'read', handler)).toBe(handler)
    await handler({})
    await flushAsync()
  ...
  it('logs write-level tools by default (writes mode)', async () => {
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('voicenotes_note_update_tags', 'write', async () => ({ content: [{ type: 'text', text: 'ok' }] }))
  ...
  it('logs read-level tools when MCP_VOICENOTES_EDIT_AUDIT_LOG=all', async () => {
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG = 'all'
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('voicenotes_note_get', 'read', async () => ({ content: [{ type: 'text', text: 'ok' }] }))
  ...
  it('records ok:false when isError:true', async () => {
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('voicenotes_note_update_tags', 'write', async () => ({ isError: true, content: [{ type: 'text', text: 'bad uuid' }] }))
    await wrapped({})
    await flushAsync()
    const event = JSON.parse((await fs.readFile(logPath, 'utf-8')).trim())
  ...
  it('records ok:false when the handler throws', async () => {
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('voicenotes_note_update_tags', 'write', async () => {
  ...
  it('skips logging entirely when MCP_VOICENOTES_EDIT_AUDIT_LOG=off', async () => {
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG = 'off'
    const { withAuditLog } = await import('./audit-log.js')
    const writeHandler = vi.fn(async (_args: unknown) => ({ content: [{ type: 'text', text: 'ok' }] }))
    expect(withAuditLog('voicenotes_note_update_tags', 'write', writeHandler)).toBe(writeHandler)
    await writeHandler({})
    await flushAsync()
    await expect(fs.access(logPath)).rejects.toThrow()
  })

  it('rejects unknown MCP_VOICENOTES_EDIT_AUDIT_LOG at config load', async () => {
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG = 'sometimes'
    await expect(import('./audit-log.js')).rejects.toThrow(/Invalid MCP_VOICENOTES_EDIT_AUDIT_LOG/)
  })
  ...
  it('handles missing content array on isError results', async () => {
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('voicenotes_note_update_tags', 'write', async () => ({ isError: true }) as unknown as { content: { type: string; text: string }[] })
    await wrapped({})
    await flushAsync()
    const event = JSON.parse((await fs.readFile(logPath, 'utf-8')).trim())
    expect(event.ok).toBe(false)
  ...
  it('rotates the audit log when it exceeds AUDIT_LOG_MAX_BYTES', async () => {
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_MAX_BYTES = '64'
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('voicenotes_note_update_tags', 'write', async () => ({ content: [{ type: 'text', text: 'ok' }] }))
  ...
  it('discards the live log when AUDIT_LOG_KEEP=0', async () => {
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_MAX_BYTES = '64'
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_KEEP = '0'
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('voicenotes_note_update_tags', 'write', async () => ({ content: [{ type: 'text', text: 'ok' }] }))
    await wrapped({ uuid: 'YRjeZkMc', tags: ['a'] })
    await flushAsync()
  ...
  it('silently absorbs write failures (writes to a non-writable parent)', async () => {
    process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_PATH = path.join(tmpDir, 'no-perms', 'audit.jsonl')
    await fs.mkdir(path.dirname(process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_PATH), { recursive: true })
    await fs.chmod(path.dirname(process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_PATH), 0o500)
    ...
    await fs.chmod(path.dirname(process.env.MCP_VOICENOTES_EDIT_AUDIT_LOG_PATH), 0o700)
  })
})
